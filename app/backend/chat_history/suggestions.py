...
class SuggestionsCosmosDB:
    def __init__(self):
        ...
        self.container_name = "suggestions"
    ...
    async def add_suggestion(self, suggestion_data: Dict[str, Any]) -> str:
        ...
        if "id" not in suggestion_data:
            suggestion_data["id"] = f"feedback-{int(time.time())}"
            
        # Store the feedback
        response = await container_client.create_item(body=suggestion_data)
        return response["id"]
